...
def find_entity(entity_name: str, property_id: str, is_head=True):
    ...
    if property_id:
        ...
        try:
            ...
            if domain or property_id in ['P31']:            
                for d in data['search']:
                    ...
                    if property_id in ['P31'] or len(set(domain).intersection(set(classes))) > 0:
                        ids.append(d['id'])
                        break  # greedy                 
...
def find_property_subject(alist: Alist):
    ...
    if alist.get(tt.TIME):
        ...
        query = """
                ...
                OPTIONAL {{ wd:{entity_id} wdt:{property_id} ?s . }}   
                SERVICE wikibase:label {{ bd:serviceParam wikibase:language "en".}}
                }} LIMIT 10000
                """.format(entity_id=entity_id, property_id=alist.get(tt.PROPERTY))
                ## hack to find inverse triple. BUG should be {...} UNION {...}
                # temp limit set at 20000
